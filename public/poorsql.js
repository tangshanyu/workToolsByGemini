...
(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.PoorSQL = {}));
	...
	var poorMansTSqlFormatter = createCommonjsModule(function (module) {
	...
	// 用 require 抓 CommonJS


	// 取出 formatSql
	const formatSql = poorMansTSqlFormatter.formatSql;

	// 掛到全域
	if (typeof window !== "undefined") {
	  window.PoorSQL = {
	    format: (sql, options) => formatSql(sql, options).text,
        formatFull: (sql, options) => formatSql(sql, options) // Expose full object for HTML access
	  };
	}

	// 給 Rollup 輸出
	var poorsqlBuild = { formatSql };
	var poorsqlBuild_1 = poorsqlBuild.formatSql;

	exports.default = poorsqlBuild;
	exports.formatSql = poorsqlBuild_1;
