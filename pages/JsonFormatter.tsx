...
const JsonFormatter: React.FC = () => {
    ...
    // --- Render Table ---
    const renderTable = () => {
        if (!parsedData) return <div className="p-8 text-gray-500 text-center text-sm">請先解析資料以產生表格</div>;
        ...
        if (Array.isArray(parsedData)) {
            ...
            // Case: Primitive root
            return <div className="p-4 text-gray-500 text-center">原始型別 ({typeof parsedData}) 不適合以表格顯示</div>;
        }

        if (rows.length === 0) return <div className="p-8 text-gray-500 text-center">空陣列 (Empty Array)</div>;
        ...
        return (
            <div className="w-full flex flex-col">
                {/* Filter Input */}
                <div className="p-2 border-b border-gray-100 dark:border-[#333] bg-gray-50 dark:bg-[#1E1E1E]">
                    <input
                        type="text"
                        placeholder="🔍 篩選資料..."
                        className="w-full px-3 py-1.5 text-sm rounded-md border border-gray-300 dark:border-[#333] bg-white dark:bg-[#141414] text-gray-800 dark:text-gray-200 focus:outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 transition-all"
                        value={tableFilter}
                ...
                <div className="w-full overflow-x-auto bg-white dark:bg-[#141414]">
                    <table className="min-w-full text-left text-sm whitespace-nowrap border-collapse">
                        <thead className="bg-gray-100 dark:bg-[#252526] border-b border-gray-200 dark:border-[#333]">
                            <tr>
                                {isArrayOfObjects && (
                                    <th className="p-3 border-r border-gray-200 dark:border-[#333] w-12 text-center text-gray-500 dark:text-gray-400 font-mono text-xs select-none">
                                        #
                                    </th>
                                )}
                                {headers.map(h => (
                                    <th
                                        ...
                                        className="p-3 font-semibold text-gray-700 dark:text-[#E8EAED] border-r border-gray-200 dark:border-[#333] min-w-[100px] cursor-pointer hover:bg-gray-200 dark:hover:bg-[#2D2D2D] transition-colors select-none group"
                                    >
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-[#333]/50">
                            {displayRows.length === 0 ? (
                                ...
                                displayRows.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-blue-50/50 dark:hover:bg-[#2D2D2D] transition-colors group">
                                        {isArrayOfObjects ? (
                                            <>
                                                <td className="p-3 text-gray-400 text-xs text-center border-r border-gray-100 dark:border-[#333] bg-gray-50 dark:bg-[#1E1E1E] group-hover:bg-blue-100/50 dark:group-hover:bg-[#2D2D2D]">
                                                    {idx + 1}
                                                </td>
                                                {headers.map(header => {
    ...
    return (
        <div className="flex flex-col h-full gap-4">
            ...
            <PageHeader
                ...
                icon="{}"
                description={
                    <span>格式化 JSON、解析 Java <code>Map.toString()</code> (格式如 <code>{`{key=val}`}</code>) 以及 CSV 格式資料</span>
                }
                controls={
                    <div className="flex bg-gray-100 dark:bg-[#252526] p-1 rounded-lg border border-gray-200 dark:border-[#333]">
                        {(['auto', 'json', 'java', 'csv'] as ParseMode[]).map((m) => (
                            <button
                                key={m}
                                onClick={() => { setMode(m); setDetectedMode(null); }}
                                className={`px-4 py-1.5 text-sm font-medium rounded-md transition-all ${mode === m
            ...
            <div className={`${viewMode === 'table' ? 'min-h-[200px]' : 'flex-[2] min-h-[200px] flex flex-col overflow-hidden'}`}>
                ...
                <div className="flex justify-between items-center mb-2 px-1">
                    <div className="flex items-center gap-3">
                        <h3 className="text-sm font-medium text-gray-700 dark:text-[#E8EAED]">輸出結果</h3>
                    ...
                    {/* View Mode Toggle */}
                    <div className="flex bg-gray-200 dark:bg-[#2D2D2D] p-0.5 rounded-lg text-xs">
                        <button
            ...
            ${viewMode === 'table' ? 'h-auto overflow-visible bg-white dark:bg-[#141414]' : 'flex-1 overflow-hidden'}
            ${error ? 'border-red-300 dark:border-red-800' : 'border-green-200 dark:border-[#333]'}
        `}>

                    {viewMode === 'text' ? (
                        <>
                            <div className={`flex justify-between items-center px-4 py-2 border-b ${error ? 'bg-red-50 dark:bg-[#2a1212] border-red-200 dark:border-red-800' : 'bg-gray-50 dark:bg-[#252526] border-gray-100 dark:border-[#333]'}`}>
                                ...
                                <button
                                    ...
                                    className="text-xs flex items-center gap-1 bg-white dark:bg-[#2D2D2D] border border-gray-200 dark:border-[#444] text-gray-700 dark:text-[#E8EAED] px-3 py-1 rounded hover:bg-gray-50 dark:hover:bg-[#333] transition-colors disabled:opacity-50"
                                >
                                    <span>📋</span> 複製
                                </button>
                            </div>
                            <div className="flex-1 p-0 relative bg-white dark:bg-[#141414] h-full">
                                <textarea
